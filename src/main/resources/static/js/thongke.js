...
phieuChart = new Chart(
  document.getElementById("phieu-chart").getContext("2d"),
  {
    type: "bar",
    data: {
      labels: [],
        ...
        y: {
          ...
          ticks: {
            stepSize: () => {
              ...
              if (max_f_d_10 <= 1) return 1;

              if (max_f_d_10 === max_i_d_10) return max_i_d_10;
...
doanhThuChart = new Chart(
  document.getElementById("doanhthu-chart").getContext("2d"),
  {
    type: "bar",
    data: {
      labels: [],
...
const generate_month = (date_table, year_text, month_text, ic_calendar) => {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    p.addEventListener("click", async (e) => {
      let value_month_text_split_number = e.target.innerText
        .split(" ")[1]
        .trim();
      let value_month_int =
        value_month_text_split_number.split("").length < 2
          ? "0" + value_month_text_split_number.toString()
          : value_month_text_split_number.toString();
...
opt_date.forEach((item) => {
  ...
  ic_calendar.addEventListener("click", (e) => {
    ...
    let month_text_red =
      month_text !== null ? month_text.classList.contains("cl-red") : false;
  ...
  item.querySelectorAll("*").forEach((i) => {
    i.addEventListener("click", (e) => {
      ...
      if (
        ev_date_table_target !== null &&
        ev_date_table_target !== date_table_in
      ) {
        ev_date_table_target.classList.add("d-none");
        ev_date_table_target.parentNode
          .querySelector(".bxs-calendar")
          .classList.remove("cl-red");
      }
